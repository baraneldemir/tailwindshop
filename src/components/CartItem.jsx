...
export default function CartItem({ id, quantity }) {
  ...
  function handleDelete() {
    if (isConfirmDelete) {
      removeFromCart(item.id); // Delete the item if already confirmed
    } else {
      ...
      <div className="flex mt-6 sm:mt-8 md:gap-6 lg:items-start xl:gap-8">
        <div className="flex-none w-full mx-auto lg:max-w-2xl xl:max-w-4xl">
          <div className="space-y-6">
            <div className="relative h-32 p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 dark:bg-gray-800 md:p-6">
              <button
                onClick={handleDelete}
                className="absolute flex flex-row transition-all duration-1000 top-3 right-3 hover:underline"
              >
              ...
              <div className="flex items-center justify-between h-full">
                <img className="object-cover w-20 h-20" src={item.imgUrl} alt={item.name} />
